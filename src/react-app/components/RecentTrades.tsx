    ...
    <div className={`${getCardBg(theme)} rounded-lg p-3 sm:p-4 border ${getCardBorder(theme)}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[#00D9C8]/10 rounded-lg flex items-center justify-center">
            <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-[#00D9C8]" />
          </div>
          <div>
        ...
        <Link 
          to="/journal" 
          className="px-3 sm:px-4 py-1.5 sm:py-2 bg-[#00D9C8]/20 hover:bg-[#00D9C8]/30 text-[#00D9C8] rounded-lg text-xs sm:text-sm font-medium transition-colors flex items-center space-x-1.5 sm:space-x-2"
        >
      ...
      {trades.length === 0 ? (
        <div className="text-center py-8 sm:py-12">
          ...
          <p className={`${getTextColor(theme, 'muted')} text-xs sm:text-sm`}>
            Start by <Link to="/journal" className="text-[#00D9C8] hover:text-[#00F5E1] transition-colors">adding your first trade</Link>
          </p>
        </div>
      ) : (
              ...
              <div className="text-right flex-shrink-0">
                {trade.is_closed && trade.pnl !== null ? (
                  <div className={`text-xs sm:text-sm font-semibold ${
                    (trade.pnl ?? 0) >= 0 ? 'text-[#00D9C8]' : 'text-[#F43F5E]'
                  }`}>
                    {(trade.pnl ?? 0) >= 0 ? '+' : ''}${(trade.pnl ?? 0).toFixed(2)}
                  </div>
                ) : (
                  <div className="flex items-center space-x-1">
                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-[#00D9C8] rounded-full animate-pulse" />
                    <span className="text-[#00D9C8] text-xs sm:text-sm font-medium">Open</span>
                  </div>
                )}
